...
$upload_file = "Gallery/";
$upload_path = "Gallery/students/";
$thumb_path  = "Gallery/students/thumb/";
...
if(isset($_POST["submit"])){
	  ...
	  $students_department =  $_POST['students_department'];
	  $students_number =  $_POST['students_number'];
	  $students_password =  $_POST['students_password'];
	  $students_country =  $_POST['students_country'];
...
$students_id = uniqid(md5(rand(10,1000)));


  $query = "INSERT INTO `gallery_photos` (`id`, `image_path`, `gallery_id`,`thumbnails`, `hash_id`, `time_created`, `time_updated`)
           VALUES (NULL, '{$full_name}', '{$gallery_id}','{$thumb_full_name}', '{$students_id}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
        ...
        if((mysqli_query($db,$query))) {//$descr_id
			...
			$des_query = "INSERT INTO `students` (`students_id`, `students_name`, `gallery_id`, `students_department`, `students_number`,`students_password`,`students_country`,`students_class`,  `photo_id`, `hash_id`, `time_created`, `time_updated`) 
			              VALUES (NULL,'$students_name','$gallery_id','$students_department', '$students_number', '$students_password','$students_country', '$students_class', '$last_id', '$students_id', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
			
            if (mysqli_query($db, $des_query)) {
				
                header("Location: gediz_students.php?id=" . $gallery_id);
            } else
			{
                echo "<script>
      alert('Error in Uploading Image!!!');
      window.location.href='gediz_students.php?id=" . $gallery_id . "';
      </script>";
      }
     }
  ...
  else header("Location: gediz_students.php");
