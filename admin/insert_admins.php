...
$upload_file = "Gallery/";
$upload_path = "Gallery/admin/";
$thumb_path  = "Gallery/admin/thumb/";
...
$students_id = uniqid(md5(rand(10,1000)));
  ...
  $query = "INSERT INTO `gallery_photos` (`id`, `image_path`, `gallery_id`,`thumbnails`, `hash_id`, `time_created`, `time_updated`)
           VALUES (NULL, '{$full_name}', '{$gallery_id}','{$thumb_full_name}', '{$admin_id}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
        ...
        if((mysqli_query($db,$query))) {//$descr_id
			...
			$des_query = "INSERT INTO `pbs_admin` (`admin_id`, `admin_name`, `gallery_id`, `admin_username`, `admin_password`,`photo_id`, `hash_id`, `time_created`, `time_updated`) 
			              VALUES (NULL,'$admin_name','$gallery_id','$admin_username', '$admin_password','$last_id', '$admin_id', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
			
            if (mysqli_query($db, $des_query)) {
				
                header("Location: gediz_admins.php?id=" . $gallery_id);
            } else
			{
                echo "<script>
      alert('Error in Uploading Image!!!');
      window.location.href='gediz_admins.php?id=" . $gallery_id . "';
      </script>";
      }
     }
  ...
  else header("Location: gediz_admins.php");
