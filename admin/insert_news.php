...
$upload_file = "Gallery/";
$upload_path = "Gallery/news/";
$thumb_path  = "Gallery/news/thumb/";
...
$news_id = uniqid(md5(rand(10,1000)));


  $query = "INSERT INTO `gallery_photos` (`id`, `image_path`, `gallery_id`,`thumbnails`, `hash_id`, `time_created`, `time_updated`)
           VALUES (NULL, '{$full_name}', '{$gallery_id}','{$thumb_full_name}', '{$news_id}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
        ...
        if((mysqli_query($db,$query))) {//$descr_id
			...
			$des_query = "INSERT INTO `latest_news` (`news_id`, `news_title`, `gallery_id`, `news_details`, `start_date`,`end_date`,`news_time`,  `photo_id`, `hash_id`, `time_created`, `time_updated`) 
			              VALUES (NULL,'$news_title','$gallery_id','$news_details', '$start_date','$end_date', '$news_time', '$last_id', '$news_id', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
			
            if (mysqli_query($db, $des_query)) {
				
                header("Location: add_events.php?id=" . $gallery_id);
            } else
			{
                echo "<script>
      alert('Error in Uploading Image!!!');
      window.location.href='add_events.php?id=" . $gallery_id . "';
      </script>";
      }
     }
